...
class AzuriteTableRequest {
  ...
  _mapFilterQueryString(filter) {
    filter = filter
      ...
      // Since we will evaluate this string we replace simple quotation marks indicating strings with template quotation marks
      .replace(/''/g, "___QUOTATION_MARK___")
      .replace(/'/g, "`")
      .replace(/___QUOTATION_MARK___/g, `'`)
